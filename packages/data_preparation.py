...
def auto_clean_dataframe(df_input: pd.DataFrame) -> Tuple[pd.DataFrame, List[Dict[str, Any]]]:
    """Auto-clean dataframe and return (cleaned_df, prep_log).

    Steps:
      1) Normalize column names; drop entirely empty columns
      2) Unify common missing tokens to NaN (object columns)
      3) Type conversions: dates, numeric; mark categoricals
      4) Handle missing: drop near-empty cols/rows, impute
      5) Winsorize numeric outliers with IQR clipping
      6) Drop duplicates and low-information columns
      7) Collapse rare categories to '__OTHER__'
    ...
    # 1) Normalize columns; drop entirely empty columns
    before = dfc.shape
    ...
    # 4) Missing: drop near-empty cols/rows + impute
    before = dfc.shape
    drop_cols: List[str] = []
    for c in list(dfc.columns):
    ...
    # 5) Winsorize numeric outliers via IQR
    before = dfc.shape
    ...
    # 6) Duplicates and low-information columns
    before = dfc.shape
